...
if (isset($_REQUEST['LO'])&&($_REQUEST['LO'] == "1")){
			session_unset();
			session_destroy();	
				}
		...
		<div id="main" class="main">
			...
			<div id="content" class="content">
				...
				<div id="contentLeft" class="contentLeft">
					...
					<?php
	  $sql  = "SELECT Count(*) FROM rants INNER JOIN type ON type.typeId = rants.rantType INNER JOIN category ON category.categoryId = rants.rantCategory INNER JOIN users ON users.userId = rants.userId  WHERE rants.userId='$author' AND rants.rantStatus = 'A'  $categoryFilter $filter $criteria ";
...
$offset = ($currentpage - 1) * $rowsperpage;
			...
			$i=1;
				 while($row = mysql_fetch_array($result, MYSQL_ASSOC))
					...
					<?php
					if ($currentpage > 1) {
					?><a href="<?php echo($_SERVER['PHP_SELF'])?>?currentpage=<?php echo($currentpage-1)?>&show=<?php echo($show)?>&category=<?php echo($category)?>&criteria=<?php echo($$forwardCriteria)?>&author=<?php echo($author)?>" target="_self">Previous</a>
					<?php
					}
					...
					?><a href="<?php echo($_SERVER['PHP_SELF'])?>?currentpage=<?php echo($currentpage+1)?>&show=<?php echo($show)?>&category=<?php echo($category)?>&criteria=<?php echo($$forwardCriteria)?>&author=<?php echo($author)?>" target="_self">Next</a>
					<?php
					}?>	
